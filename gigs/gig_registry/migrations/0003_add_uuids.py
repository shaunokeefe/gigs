...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for band in orm.Band.objects.all():
            info = ' '.join([band.name])
            band.uuid = hashlib.sha1(info).hexdigest() 
            band.save()

        for l in orm.Location.objects.all():
            info = ' '.join([l.street_address, l.suburb, l.state, l.post_code, l.country])
            l.uuid = hashlib.sha1(info).hexdigest() 
            l.save()

        for venue in orm.Venue.objects.all():
            venue.uuid = venue.uid
            if not venue.uuid:
                venue.uuid = hashlib.sha1(venue.name).hexdigest() 
            venue.save()
        
        for gig in orm.Gig.objects.all():
            info = ' '.join([str(gig.start), gig.venue.uuid])
            gig.uuid = hashlib.sha1(info).hexdigest() 
            gig.save()
